...
# ----------------------------------------------------------------------
# REPORTS — #5 (lives at top-level /reports/, see home/urls.py, since
# /admin/ is already Django's real admin site)
# ----------------------------------------------------------------------
class ReportsView(StaffGroupRequiredMixin, View):
    """
    Sales/profit dashboard. Revenue and quantity come straight from
    FaktorItem (the price actually charged at sale time). Cost is
    approximated from the product's *current* price_for_company, since
    FaktorItem doesn't record a cost-basis snapshot at sale time — if
    company pricing changes over time this will retroactively shift
    historical profit figures. Flagging this as a known simplification
    rather than silently presenting it as exact.
    """
    template_name = "faktor/reports.html"

    def get(self, request):
        sell_items = FaktorItem.objects.filter(sell__isnull=False)

        total_revenue = sum(i.total_price for i in sell_items)
        total_cost = sum(i.quantity * i.product.price_for_company for i in sell_items)
        total_profit = total_revenue - total_cost

        # Daily sales for the last 30 days
        since = timezone.now().date() - timedelta(days=30)
        daily = (
            sell_items.filter(sell__date__gte=since)
            .values('sell__date')
            .annotate(day_total=Sum(F('unit_price') * F('quantity')))
            .order_by('sell__date')
        )
        daily_labels = [str(row['sell__date']) for row in daily]
        daily_values = [float(row['day_total'] or 0) for row in daily]

        # Top products by revenue
        by_product = {}
        for item in sell_items.select_related('product'):
            key = item.product.name
            by_product.setdefault(key, {'revenue': 0, 'qty': 0})
            by_product[key]['revenue'] += float(item.total_price)
            by_product[key]['qty'] += item.quantity
        top_products = sorted(by_product.items(), key=lambda kv: -kv[1]['revenue'])[:10]
        ...
        context = {
            ...
            'top_product_labels': [name for name, _ in top_products],
            'top_product_values': [v['revenue'] for _, v in top_products],
        }
        return render(request, self.template_name, context)
